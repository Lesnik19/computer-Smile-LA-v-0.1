...
Начнём определения. Регистр - это ячейка памяти, с которой работает компьютер. У меня все регистры на 8 бит, поэтому мой ПК является 8-битным. Регистры есть специального назначения (нужны для работы ПК) и общего назначения (РОН). В этой версии Smile LA НОК всего 1 - он называется Акамулятором (обощначается ACC). РОН нужны для данных, с которыми мы работаем. 

Помимо ACC для программирования нужны флаги - это специально выделеные биты, которые изменяют свои значения при математических операциях. С их помощью создаётся ветвление кода. В моём ПК их всего 2 - Z (становится единицей, если результат операции равен 0, иначе становится нулём) и C (становится единицей, если результат операции не вмещается в байт и самый левый бит выходит за его рамки, иначе становится нулём).

Компьютер по очереди берёт команды из памяти, начиная с начала, и выполняет их.
